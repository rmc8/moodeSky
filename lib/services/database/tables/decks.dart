...
class Decks extends Table {
  ...
  TextColumn get deckId =>
      text().withLength(min: 1, max: 100)(); // Unique deck identifier
  TextColumn get title => text().withLength(min: 1, max: 200)();
  TextColumn get description => text().nullable()();

  // Deck type and configuration
  TextColumn get deckType =>
      text()(); // 'home', 'notifications', 'search', 'list', 'profile', 'thread', 'custom_feed', 'local', 'hashtag', 'mentions'
  TextColumn get targetIdentifier =>
      text().nullable()(); // List URI, user DID, hashtag, search query, etc.
  TextColumn get config =>
      text().nullable()(); // JSON configuration for deck-specific settings

  // Account association
  TextColumn get accountDid => text()
      .nullable()(); // Associated account DID (null for cross-account decks)
  BoolColumn get isCrossAccount =>
      boolean().withDefault(const Constant(false))(); // Multi-account deck flag
  TextColumn get accountDids =>
      text().nullable()(); // JSON array of account DIDs for cross-account decks

  // Display and layout
  IntColumn get deckOrder =>
      integer().withDefault(const Constant(0))(); // Display order
  IntColumn get width =>
      integer().nullable()(); // Column width (pixels, for desktop)
  ...
  TextColumn get filters =>
      text().nullable()(); // JSON configuration for content filters
  ...
  TextColumn get contentLanguages =>
      text().nullable()(); // JSON array of preferred languages

  // Refresh and sync settings
  BoolColumn get autoRefresh => boolean().withDefault(const Constant(true))();
  IntColumn get refreshInterval =>
      integer().withDefault(const Constant(60))(); // Seconds
  ...
  TextColumn get notificationConfig =>
      text().nullable()(); // JSON notification settings
