...
class ServerDiscoveryService {
  ...
  static Future<ServerConfig> discoverServer(
    ...
    try {
      ...
      return ServerConfig(
        ...
        supportsAppPasswords: _supportsFeature(serverInfo, 'app_passwords', defaultValue: true),
  ...
  static Future<ServerConfig> validateServer(ServerConfig config) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${config.serviceUrl}/xrpc/com.atproto.server.describeServer'),
        headers: {
          'User-Agent': 'MoodeSky/1.0',
          ...config.headers,
        },
      ...
      if (response.statusCode == 200) {
        ...
        final isCompatible = _isCompatibleVersion(version, config.protocolVersion);
  ...
  static Future<Map<String, bool>> testAuthMethods(ServerConfig config) async {
    ...
    try {
      ...
      final sessionResponse = await http.get(
        Uri.parse('${config.serviceUrl}/xrpc/com.atproto.server.createSession'),
        headers: {'User-Agent': 'MoodeSky/1.0'},
      ).timeout(config.timeout);
      
      // If we get a method-not-allowed or similar, the endpoint exists
      results['app_password'] = sessionResponse.statusCode == 405 || 
      ...
      if (config.supportsOAuth) {
        final oauthResponse = await http.get(
          Uri.parse('${config.serviceUrl}/.well-known/oauth-authorization-server'),
          headers: {'User-Agent': 'MoodeSky/1.0'},
        ).timeout(config.timeout);
  ...
  static bool _isCompatibleVersion(String? serverVersion, String clientVersion) {
